...
//   useEffect(() => {
//     fetch("https://myportfolio-nyjh.onrender.com/api/skills") // Adjust based on your API route
//       .then((res) => res.json())
...
const Skills = () => {
  ...
  return (
    <section className="relative w-full bg-[#0A0A0A] text-white py-20 overflow-hidden">
      ...
      {loading ? (
        <div className="text-center text-gray-400 mt-12">Loading skills...</div>
      ) : skills.length === 0 ? (
        <div className="text-center text-gray-400 mt-12">No skills available.</div>
      ) : (
        <motion.div
          className="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3 px-6"
          initial={{ opacity: 0 }}
